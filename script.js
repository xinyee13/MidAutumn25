// --- JITTER FIX: Lock the height to prevent mobile browser toolbar shifting ---
function setViewportHeight() {
    const vh = window.innerHeight * 0.01;
    document.documentElement.style.setProperty('--vh', `${vh}px`);
    
    // Explicitly set the body and night-sky container height in pixels
    const actualHeight = window.innerHeight;
    document.body.style.height = `${actualHeight}px`;
    document.querySelector('.night-sky').style.height = `${actualHeight}px`;
    document.querySelector('.background-image-layer').style.height = `${actualHeight}px`;
}

// Set on load and on any resize event
setViewportHeight();
